...
declare const Buffer: {
  from: (input: string | ArrayBuffer, encoding?: string) => Uint8Array;
...
const runningHubAppRunEndpoint = `${runningHubBaseUrl}/task/openapi/ai-app/run`;
...
async function readRawBody(request: RequestLike) {
...
function resolveOpenAiChatEndpoint(baseUrlKey = "OPENAI_BASE_URL") {
  const configuredBaseUrl =
    envValue(baseUrlKey) ||
    envValue("OPENAI_BASE_URL") ||
    envValue("CONCEPT_OUTLINE_ARK_CHAT_ENDPOINT") ||
...
function resolveOpenAiResponsesEndpoint(baseUrlKey = "OPENAI_DOCUMENT_OUTPUT_BASE_URL") {
  const configuredBaseUrl =
    envValue(baseUrlKey) ||
    envValue("OPENAI_BASE_URL") ||
    envValue("CONCEPT_TEXT_IMAGE_ENDPOINT") ||
    openAiDefaultBaseUrl;
...
function resolveDocumentOutputTimeoutMs() {
  const configured = envValue("OPENAI_DOCUMENT_OUTPUT_TIMEOUT_MS");
...
function resolveRunningHubUpscaleWebappId() {
  const configuredAppPath =
    envValue("RUNNINGHUB_UPSCALE_APP_PATH") || runningHubDefaultUpscaleAppPath;
  const appIdFromPath = configuredAppPath.match(/(?:run\/ai-app|ai-detail)\/(\d+)/)?.[1];

  return (
    appIdFromPath ||
    envValue("RUNNINGHUB_UPSCALE_WEBAPP_ID") ||
...
function extractDocumentOutputResult(payload: unknown) {
  return {
    ...
    text: extractDocumentOutputText(payload),
    images: collectDocumentOutputImages(payload),
  };
}
...
function normalizeAgentAudio(value: unknown) {
  ...
  return items
    .flatMap((item) => {
      if (typeof item === "string") {
        return [{ audioUrl: item, audioBase64: "", label: "" }];
      }
...
function buildAgentPrompt({
  ...
  materials: ReturnType<typeof normalizeProjectMaterialInputs>;
  agentImages: ReturnType<typeof normalizeAgentImages>;
  agentAudio: ReturnType<typeof normalizeAgentAudio>;
}) {
  ...
  const projectText = project && !isOutlineTask && !isDocumentOutputTask
    ? isDocumentOutputTask
      ...
      : `\n\n当前项目：${String(project.name || "未命名项目")}；类型：${String(
          project.type || "未填写",
        )}；客户：${String(project.client || "未填写")}。`
    : "";
  const materialText = materials.length
    ? isDocumentOutputTask
      ? `\n已附带项目资料源文件：${materials
  ...
  const imageContext = agentImages.length
    ...
    : "";
  const audioContext = agentAudio.length
    ? `\n已附带麦克风语音：${agentAudio
  ...
  return isOutlineTask
    ? withZerlumLandscapeContext(basePrompt, { forGeneration: false })
    : withZerlumLandscapeContext(basePrompt);
}
...
function extractRunningStatus(payload: unknown) {
  const explicitStatus = findScalarByKey(payload, [
    "taskStatus",
    "status",
    "state",
    "phase",
...
async function uploadRunningHubModelImage(apiKey: string, imageUrl: string) {
  ...
  formData.append(
    "file",
    new Blob([toBlobPart(image.buffer)], { type: image.mimeType }),
    image.fileName,
  );
...
function normalizeVideoResolution(value: unknown) {
  ...
  if (["480p", "720p", "1080p"].includes(normalized)) {
    return normalized;
  }

  if (["2k", "4k"].includes(normalized)) {
    return "1080p";
  }
...
function resolveVideoWaitTimeoutMs() {
  const configured = envValue("ARK_VIDEO_WAIT_TIMEOUT_MS");
  const parsed = configured ? Number(configured) : NaN;
...
function resolveVideoPollIntervalMs() {
  const configured = envValue("ARK_VIDEO_POLL_INTERVAL_MS");
  const parsed = configured ? Number(configured) : NaN;
...
function extractArkVideoResult(payload: unknown) {
  ...
  return {
    ...
    status: extractRunningStatus(payload),
    videoUrl,
    outputText,
  };
}
...
async function pipeResponseBody(upstream: Response, response: ResponseLike) {
  ...
  for await (const chunk of upstream.body as unknown as AsyncIterable<unknown>) {
...
export async function handleZerlumAgent(request: RequestLike, response: ResponseLike) {
  ...
  try {
    ...
    const enrichedMessage = buildAgentPrompt({
      message,
      view,
      agentTask,
      project:
    ...
    const agentModel = isDocumentOutputTask
      ? envValue("OPENAI_DOCUMENT_OUTPUT_MODEL") ||
        openAiDefaultDocumentOutputModel
      : useOpenAiChat
        ? isOutlineTask
          ? envValue("OPENAI_OUTLINE_MODEL", "CONCEPT_OUTLINE_ARK_CHAT_MODEL") ||
            envValue("OPENAI_AGENT_MODEL") ||
      ...
      if (isDocumentOutputTask && upstreamController?.signal.aborted) {
        sendJson(response, 504, {
          error: `image2 请求超时：超过 ${Math.round(
            resolveDocumentOutputTimeoutMs() / 1000,
          )} 秒没有返回。`,
        });
        return;
      }
...
export async function handleZerlumPrompt(request: RequestLike, response: ResponseLike) {
  if (request.method !== "POST") {
    sendJson(response, 405, { error: "Method not allowed" });
    return;
  }
...
export async function handleZerlumImage(request: RequestLike, response: ResponseLike) {
  if (request.method !== "POST") {
    sendJson(response, 405, { error: "Method not allowed" });
    return;
  }
  ...
  try {
    ...
    const imageEndpoint =
      envValue(
        ...
        "NIGHT_RENDER_IMAGE_ENDPOINT",
        "NIGHT_RENDER_BANANA_IMAGE_ENDPOINT",
...
export async function handleZerlumVideo(request: RequestLike, response: ResponseLike) {
  if (request.method !== "POST") {
    sendJson(response, 405, { error: "Method not allowed" });
    return;
  }
  ...
  try {
    ...
    const videoApiKey = envValue(
      "ARK_VIDEO_API_KEY",
      "ARK_API_KEY",
      "ARK_RESPONSES_API_KEY",
    );
    ...
    const videoModel = envValue("ARK_VIDEO_MODEL") || arkDefaultVideoModel;
    const endpoint = (
      envValue("ARK_VIDEO_TASKS_ENDPOINT") || arkVideoTasksEndpoint
    ).replace(/\/+$/, "");
    ...
    sendJson(response, 200, {
      ...
      outputText:
        ...
        (creationResult.videoUrl ? "视频已生成。" : "视频任务已提交，正在生成中。"),
      model: videoModel,
      ratio,
      resolution,
      duration,
...
function getRequestQueryParam(request: RequestLike, name: string) {
...
export async function handleZerlumVideoStatus(
  request: RequestLike,
  response: ResponseLike,
) {
  if (request.method !== "GET" && request.method !== "POST") {
  ...
  try {
    const body =
      request.method === "POST"
        ? ((await readJsonBody(request)) as Record<string, unknown>)
        : {};
    const taskId = String(body.taskId ?? getRequestQueryParam(request, "taskId")).trim();
